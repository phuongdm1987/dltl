...
@section('content')
	...
	{{ Form::open(['route' => 'post.place.create', 'class' => 'form-horizontal', 'method' => 'POST', 'role' => 'form', 'enctype' => 'multipart/form-data']) }}
		<div class="form-group {{ $errors->has('pla_name') ? 'has-error' : '' }}">
			<label for="pla_name" class="col-sm-3 control-label">Tên địa danh</label>
			<div class="col-sm-6">
				{{ Form::text('pla_name', Input::get('pla_name'), ['class' => 'form-control', 'id' => 'pla_name', 'placeholder' => 'Nhập tên địa danh']) }}
		...
		<div class="form-group {{ $errors->has('pla_sector') ? 'has-error' : '' }}">
			...
			<div class="col-sm-3">
				{{ Form::select('pla_sector',  [1 => 'Trong nước', 2 => 'Nước ngoài'], Input::old('pla_sector'), ['class' => 'form-control']) }}
			...
			<div class="col-sm-3">
				<select name="pla_country_id" class="form-control">
					@foreach($countries as $country)
						<option value="{{ $country->cou_id }}" {{  Input::old('pla_country_id') == $country->cou_id ? 'selected="selected"' : '' }}>{{ $country->cou_name }}</option>
					@endforeach
				</select>
			...
			<div class="col-sm-3">
				<select name="pla_type" class="form-control">
					...
					@foreach($cplaces as $cplace)
						<option value="{{ $cplace->ctp_id }}" {{  Input::old('pla_type') == $cplace->ctp_id ? 'selected="selected"' : '' }}>{{ $cplace->ctp_name }}</option>
					@endforeach
				</select>
			...
			<div class="col-sm-3">
				<select name="pla_city_id" class="form-control" id="pla_city_id">
					...
					@foreach($cities as $cit_id => $cit_name)
						<option value="{{ $cit_id }}" {{  Input::old('pla_city_id') == $cit_id ? 'selected="selected"' : '' }}>{{ $cit_name }}</option>
					@endforeach
				</select>
			...
			<div class="col-sm-3">
				<select name="pla_district_id" class="form-control" id="pla_district_id">
					...
					@foreach($districts as $district)
						<option value="{{ $district->cit_id }}" {{  Input::old('pla_district_id') == $district->cit_id ? 'selected="selected"' : '' }}>{{ $district->cit_name }}</option>
					@endforeach
				<select>
		...
		<div class="form-group {{ $errors->has('pla_longitude') ? 'has-error' : '' }}">
			...
			<div class="col-sm-3">
				{{ Form::text('pla_longitude', Input::get('pla_longitude'), ['class' => 'form-control', 'id' => 'pla_longitude', 'placeholder' => 'Nhập kinh độ trên google maps']) }}
		...
		<div class="form-group {{ $errors->has('pla_latitude') ? 'has-error' : '' }}">
			...
			<div class="col-sm-3">
				{{ Form::text('pla_latitude', Input::get('pla_latitude'), ['class' => 'form-control', 'id' => 'pla_latitude', 'placeholder' => 'Nhập vĩ độ trên google maps']) }}
		...
		<div class="form-group {{ $errors->has('pla_content') ? 'has-error' : '' }}">
			...
			<div class="col-sm-6">
				{{ Form::textarea('pla_content', Input::old('pla_content'), ['class' => 'form-control content', 'id' => 'pla_content', 'placeholder' => 'Nhập thông tin mô tả về địa danh']) }}
