...
<form class="form-horizontal" method="post" action="" autocomplete="off" role="form" enctype="multipart/form-data">
   ...
   <div class="tab-content">
         ...
         <div class="col-sm-6">
            <select name="type" id="type" class="form-control">
               ...
               @foreach($types as $key => $type)
                  <option value="{{ $key }}" {{ $key == Input::old('type', $category->type) ? 'selected' : '' }}>{{ $type }}</option>
               @endforeach
            </select>
         ...
         <div class="col-sm-6">
            <select name="parents" id="parents" class="form-control">
               ...
               @foreach ($categories as $cate)
                  <option value="{{ $cate->id }}" {{ $cate->id == Input::old('parents', $category->parents) ? 'selected' : '' }}>
      ...
      <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
         ...
         <div class="col-sm-6">
            <input type="text" class="form-control" name="name" id="name" value="{{ Input::old('name', $category->name) }}"/>
      ...
      <div class="form-group {{ $errors->has('cat_background') ? 'has-error' : ''}}">
         ...
         <div class="col-sm-6">
            <input type="text" class="form-control" name="cat_background" id="cat_background" value="{{ Input::old('cat_background', $category->cat_background) }}"/>
      ...
      <div class="form-group {{ $errors->has('cat_icon') ? 'has-error' : ''}}">
         ...
         <div class="col-sm-6 col-sm-offset-2" style="margin-top:10px;">
            <img src="{{ asset('/upload/categories/sm_'. $category->cat_icon) }}" alt="{{ $category->name }}">
         </div>
      </div>
      ...
      <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
         ...
         <div class="col-sm-6">
            <textarea name="description" id="description" rows="5" class="form-control">{{ Input::old('description', $category->description) }}</textarea>
