...
@section('content')
	<tr>
		<td style="font-weight:bold; font-size:18px; padding:20px 0px; background-color: #fff;" align="center"> ĐẶT TOUR THÀNH CÔNG</td>
	</tr>
	<tr>
		<td style="background-color: #fff; padding: 0 25px;">
			<p style="margin:0px 0px 10px 0px;">Chào <strong> {{ $booking->getCustomerName() }} </strong></p>
			<p style="margin:0px 0px 10px 0px;">Cảm ơn Quý khách đã lựa chọn dịch vụ của <b>{{ SITE_NAME }}</b> cho chuyến đi của mình!</p>
			<p style="margin:0px 0px 10px 0px;">
				Nhân viên tư vấn của <b>{{ $user->fullName() }}</b> sẽ liên lạc với Quý khách trong vòng 24h trong giờ hành chính từ thứ 2 đến thứ 7 kể từ khi Quý khách gửi yêu cầu đặt tour trên <b>{{ SITE_NAME }}</b>.
			</p>
			<p style="margin:0px 0px 10px 0px;">Việc thanh toán sẽ được tiến hành sau khi Quý Khách nhận được xác nhận từ <b>{{ $user->fullName() }}</b>.</p>
			<p style="margin:0px 0px 10px 0px;">
				Ghi chú: Mọi yêu cầu sẽ được đáp ứng tùy thuộc vào điều kiện của tour trong thông tin chi tiết của tour <a href="{{ route($tourDetail->getUrl()) }}">tại đây.</a>
			</p>
			<p style="margin:0px 0px 10px 0px; font-style: italic;">Cám ơn quý khách đã đặt tour tại {{ SITE_NAME }}.com!</p>
		</td>
	</tr>
	<tr>
		<td>
	...
	{{-- <tr>
		<td style="background-color: #fff; padding: 0 25px;">
			...
			<p style="margin:0px;">
				 Trường hợp Quý khách muốn xác nhận ngay, vui lòng liên hệ với {{ SITE_NAME }} theo Hotline:  <b>Nguyễn Travel: 0973016636</b>
			</p>
		</td>
	</tr> --}}
@stop